...
\begin{enumerate}
	...
	\[ \mf{A}\mf{B}\mf{C} = \sum_{i=1}^p \sum_{j=1}^q b_ij \mf{a}_i \tilde{\mf{b}}_j^\top \]
	...
	\item Show that the diagnol elements of a square matrix $\mf{A}$, such that $\mf{A}^\top = -\mf{A}$ are zero. These are \textit{skew-symmetric} matrices.
	
	\item Show that the product of two skew-symmetric matrices is symmetric.
	
	\item Show that the inverse of a triangular matrix is also triangular.
\end{enumerate}
